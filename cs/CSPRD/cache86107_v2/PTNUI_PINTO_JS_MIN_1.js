var PT_ToastMsg = function(msg, evt) {
 ...
 try {
 var objFrame = top.frames['TargetContent']; if (objFrame && (typeof objFrame.psConfirm2 == "function") && (typeof modalCloseUrl != 'undefined' && modalCloseUrl)) {
 objFrame.psConfirm2("Message", msg, "OKANDEVT", pinEvt); } else {
 window.psConfirmClassic("Message", msg, "OK"); }
 } catch(e) {
...
var PTPinTo = {
 ...
 var pinLabel = ajaxURL.match('&crefLabel=([^&]*)'); pinLabel = (pinLabel && (pinLabel.length > 1)) ? PTPinTo.EscapeToHTML(decodeURIComponent(pinLabel[1])) : ""; var inpPin = document.getElementById("lpNewLabel"); inpPin.textContent = pinLabel; var hpList = document.getElementById('ptlplist'); if (hpList) {
 ...
 pCrefID = this.TargetFrame.szPinCrefID; pLabel = this.TargetFrame.szPinCrefLabel; pLabel=PTPinTo.DecodeHTML(pLabel);  }
 } catch(e) {}
 ...
 if ((loc == "FAV") && (pCrefID == "PT_PORTAL_CREF_ADM_GBL" || pCrefID == "PT_PORTAL_FLDR_ADM_GBL") 
 && (ptUtil.id("ptifrmatfname") != null) && (ptUtil.id("ptifrmatfdescr") != null))
 {
 pCrefID = ptUtil.id("ptifrmatfname").value; pLabel = ptUtil.id("ptifrmatfdescr").value; pin_url = this.TargetFrame.strCurrUrl
 } else {
 var strReq, strcurr; if (this.TargetFrame.strReqURL.indexOf("?") != -1)
 strReq = this.TargetFrame.strReqURL.substring(0, this.TargetFrame.strReqURL.indexOf("?")); else
 strReq = this.TargetFrame.strReqURL; if (this.TargetFrame.strCurrUrl.indexOf("?") != -1)
 strcurr = this.TargetFrame.strCurrUrl.substring(0, this.TargetFrame.strCurrUrl.indexOf("?")); else
 strcurr = this.TargetFrame.strCurrUrl; if (strcurr != "" && strReq != "" && strcurr == strReq)
 pin_url = this.TargetFrame.strReqURL; }
 } else {
 if (!pCrefID && (pin_url.toLowerCase().indexOf("isfolder=true") != -1)) {
 pCrefID = ptUtil.id("ptifrmatfname").value; if (!isCrossDomain(this.TargetFrame) && (typeof this.TargetFrame.strCurrUrl !== 'undefined'))
 pin_url = this.TargetFrame.strCurrUrl; }
 else
 ...
 function () {
 try {
 var rcd = eval(this.req.responseText); if ((rcd.length == 1) && ((rcd[0].ERROR == "NB") || (rcd[0].ERROR == "FAV")|| (rcd[0].ERROR == "SID"))) {
